    ...
    async fetchOrder(id, symbol) {
        ...
        try {
            ...
            if (mercadoNetworkError(code, error.response?.status))
                ...
                console.log(`${symbol} ORDER NOT FOUND (ID: ${id}), ERROR:`, error.response);
                throw new ccxt.NetworkError(`${code} ORDER ID: ${id}`);
            }
            else
                throw error;
        }

        //Here we are assuming that mercado bitcoin has a bug where for some unknown reason it returns a "ORDER_NOT_FOUND" error even if there is
        //an order with the provided ID, since the ID is generated by them it makes no sense saying the order doesn't exists, so i believe its a bug.
        //The same applies to cancelOrder()
    }
